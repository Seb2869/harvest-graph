...
import { ERC20 } from '../generated/UsdcPlasmaVault/ERC20';
import { pushPlasmaVault } from './types/TotalTvlUtils';
...
export function handleTransfer(event: Transfer): void {
  ...
  if (vault == null) {
    ...
    pushPlasmaVault(event.address.toHexString(), event.block);
  }
  ...
  for (let i = 0; i < fuses.length; i++) {
    ...
    const hVault = loadOrCreateVault(Address.fromString(pVault), event.block);
    if (hVault != null) {
      underlyingDecimal = hVault.decimal.toI32();
      break;
    }
  }
  const price = getPriceForCoin(Address.fromString(vault.id), event.block.number.toI32()).divDecimal(BD_18);
...
export function handleMarketBalancesUpdated(event: MarketBalancesUpdated): void {
  ...
  vaultHistory.priceUnderlying = getPriceForCoin(Address.fromString(vault.id), event.block.number.toI32()).divDecimal(BD_18);
